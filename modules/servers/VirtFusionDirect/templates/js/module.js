...
var _vfServerOsName = null;   // cached from vfServerData; used by vfResetServerPassword
var _vfServerStatus = null;  // cached from vfServerData; drives adaptive polling interval
...
function vfShowAlert(alertDiv, type, message) {
    ...
    var msgSpan = $('<span>').css({
        'margin-right': '20px',
        'display': 'inline-block'
    }).text(message);
    
    alertDiv.empty().append(closeBtn).append(msgSpan);
    alertDiv.show();
}
...
function vfServerData(serviceId, systemUrl) {
    ...
    $.ajax({
        ...
        if (response.success) {
            ...
            _vfServerOsName = data.osName || null;
            ...
            // Update status badge — maps VirtFusion server.state to badge colour
            var statusBadge = $("#vf-status-badge");
            var status = (response.data.status || "unknown").toLowerCase();
            _vfServerStatus = status;
            statusBadge.text(status.charAt(0).toUpperCase() + status.slice(1));
            statusBadge.removeClass("vf-badge-active vf-badge-suspended vf-badge-awaiting vf-badge-building vf-badge-failed");
            if (status === "complete") {
                statusBadge.addClass("vf-badge-active");
            } else if (status === "suspended") {
                statusBadge.addClass("vf-badge-suspended");
            } else if (status === "failed") {
                statusBadge.addClass("vf-badge-failed");
            } else if (status === "building" || status === "queued") {
                statusBadge.addClass("vf-badge-building");
            } else {
                // allocated, unknown, or any future state
                statusBadge.addClass("vf-badge-awaiting");
            }

            // Disable action buttons while server is not in a ready state
            var notReadyStates = ["building", "queued", "allocated", "failed"];
            vfApplyServerStateGating(notReadyStates.indexOf(status) !== -1, status);
...
/**
 * Disable/enable action buttons based on whether the server is in a ready state.
 * Called from vfServerData after every poll. States that block actions:
 *   building, queued, allocated, failed
 * State "complete" re-enables all buttons.
 */
function vfApplyServerStateGating(disable, status) {
    var reason = disable
        ? "Server is " + status + " — actions unavailable until provisioning completes"
        : "";

    // Power buttons
    ["#vf-power-boot", "#vf-power-restart", "#vf-power-shutdown", "#vf-power-poweroff"].forEach(function(sel) {
        $(sel).prop("disabled", disable).attr("title", disable ? reason : "");
        $(sel).toggleClass("vf-btn-server-gated", disable);
    });

    // VNC + Settings dropdown toggle
    ["#vf-vnc-button", "#vfSettingsDropdown"].forEach(function(sel) {
        $(sel).prop("disabled", disable).attr("title", disable ? reason : "");
        $(sel).toggleClass("vf-btn-server-gated", disable);
    });

    // Dropdown menu items (<a> tags ignore disabled prop — use class + aria)
    var dropItems = $("#vfSettingsDropdown").next(".dropdown-menu").find(".dropdown-item");
    if (disable) {
        dropItems.addClass("vf-btn-server-gated disabled").attr("aria-disabled", "true").attr("tabindex", "-1");
    } else {
        dropItems.removeClass("vf-btn-server-gated disabled").removeAttr("aria-disabled").removeAttr("tabindex");
    }
}
...
function vfResetServerPassword(serviceId, systemUrl) {
    var isWindows = _vfServerOsName && /windows/i.test(_vfServerOsName);
    var user = isWindows ? "Administrator" : "root";
...
(function _vfLiveStatsRefresh() {
    var READY_MS    = 30000; // normal interval when server is complete
    var BUILDING_MS = 10000; // aggressive interval when building/queued/allocated/failed
    var NOT_READY   = ["building", "queued", "allocated", "failed"];

    var timer = null;
    var serviceId = null, systemUrl = null;

    function schedule() {
        var delay = (NOT_READY.indexOf(_vfServerStatus) !== -1) ? BUILDING_MS : READY_MS;
        timer = setTimeout(tick, delay);
    }

    function tick() {
        if (!serviceId || document.hidden) { schedule(); return; }

        // Non-ready state: full vfServerData re-poll so badge + button gating update.
        // vfServerData success calls vfStartLiveStatsRefresh which resets the timer;
        // schedule() here is a safety fallback in case the request fails.
        if (NOT_READY.indexOf(_vfServerStatus) !== -1) {
            vfServerData(serviceId, systemUrl);
            schedule();
            return;
        }

        // Ready state: live stats only — only while Resources tab is visible.
        var container = document.getElementById("vf-livestats-container");
        if (!container || container.offsetParent === null) { schedule(); return; }
